...
export const createLoan= async(data) => {
  try {
    const user = await axios.get(`http://localhost/api/users/${data.user_id}`, {
      timeout: 5000,
    });

    const book = await axios.get(`http://localhost/api/books/${data.book_id}`, {
      timeout: 5000,
    });
    ...
    await axios.patch(`http://localhost/api/books/${data.book_id}/availability`,
      {
        available_copies: 1,
        operation: 'decrement',
      },
...
export const returnBook = async (loanId) => {
  try {
    ...
    await axios.patch(
      `http://localhost/api/books/${loan.bookId}/availability`,
      {
        available_copies: 1,
        operation: 'increment',
      },
    ...
    const enrichedLoans = await Promise.all(
      loans.map(async (loan) => {
        const [bookResponse, userResponse] = await Promise.all([
          axios.get(`http://localhost/api/books/${loan.bookId}`, { timeout: 5000 }),
          axios.get(`http://localhost/api/users/${loan.userId}`, { timeout: 5000 }),
        ]);
...
//   try {
//     const loan = await prisma.issue.findUnique({
//       where: { id },
//     });
